...
Your team's repo (`madd-f26/madd-team-<letter>`) already has a `6-final-analysis/` folder seeded with a `report.qmd` starter file -- write your report there. The starter comes with some text and code explaining how to use it - you should delete this code & text as it is only for explanatory purposes. Be sure to adjust the content in the YAML:
...
1. Download the survey response data from your Supabase account. You can import the data directly using the `sd_get_data()` function (see the [documentation page](https://surveydown.org/docs/fetching-data) for details). Save the raw data in the "data" folder in your team repo's `6-final-analysis/` folder.
