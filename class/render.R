...
# Build the 1-2 page class summary (skipped for weeks that don't have one yet)
if (file.exists("summary.qmd")) {
...
# Expand folders and drop anything generated by running a survey locally --
# above all a `.env` file, which holds database credentials.
practice_files <- unlist(lapply(practice_files, function(f) {
  if (dir.exists(f)) {
    list.files(f, recursive = TRUE, full.names = TRUE, all.files = TRUE)
  } else {
    f
  }
}))
generated <- grepl(
  "(^|/)(\\.env|_survey|preview_data\\.csv|local_data\\.csv|rsconnect|\\.DS_Store)(/|$)",
  practice_files
)
practice_files <- practice_files[!generated]
