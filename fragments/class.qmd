...
# never advertise a zip that class/render.R didn't build. Same idea for the
# one-page class summary, which only exists for decks that have a summary.qmd.
has_notes <- file.exists(here::here("class", path_notes))
has_summary <- file.exists(here::here("class", path_summary))
```
...
# Each button needs its own paragraph, so separate them with a blank line --
# consecutive anchors in one block get merged into a single <p> and wrap inline.
if (has_summary) {
  cat(
    '<a href="', path_summary,
    '" class="btn-notes"><i class="fas fa-file-pdf"></i> Class summary</a>\n\n',
    sep = ''
  )
}
